...
import { isColliding, getOBBCollision, getCircleCollision, getContactPoint } from "./CollisionSystem.js";


export class EnemySystem{
  constructor(state){
    this.state = state;
    this.maxDrones = 2; // maximum number of drones allowed
  }
  /**
  ...
  checkEnemyBulletCollision(){
    ...
    for(let i = this.state.enemyBullets.length - 1; i >= 0; i--){
      ...
      if(isColliding(player, bullet)){
        ...
        // Source: https://www.w3schools.com/jsref/jsref_splice.asp
   ...
   * If they collide, the drone is instantly destroyed 
   * and transfers its "momentum" into the asteroid, 
   * knocking it away. Afterwards, a new drone spawns.
   */
  checkAsteroidDroneCollision(){
    const drones = this.state.drones; 
    const asteroids = this.state.asteroids; 
    // reverse loop because drones can be deleted here
    for(let i = drones.length - 1; i >= 0; i--){
      let isHit = false;
      let impactNormal = null; 
      let impactPoint = null; 
      const drone = drones[i];
      drone.width = drone.width || (drone.radius * 2);
      drone.height = drone.height || (drone.radius * 2);
      for(const asteroid of asteroids){
        // route the correct collision math based on debug toggles
        if(this.state.debug.useCircleColliders){
          const collisionData = getCircleCollision(drone, asteroid);
          if(collisionData !== null){
            isHit = true; 
            impactNormal = collisionData.normal; 
            impactPoint = drone.position.add(impactNormal.scale(drone.radius));
          }
        }
        else{
          const collisionData = getOBBCollision(drone, asteroid);
          if(collisionData !== null){
            isHit = true;
            impactNormal = collisionData.normal; 
            impactPoint = getContactPoint(drone, asteroid, impactNormal);
          }
        }
        if(isHit){
          if(this.state.debug.rigidBodiesEnabled && asteroid.applyForceAtPoint){
            const speed = drone.speed || 80; 
            // calculates how hard the drone hit the asteroid using the speed 
            // and a pre-defined factor.
            const forceMagnitude = Math.max(speed * 2000, 20000);
            // force vector points opposite to the collision normal.
            const forceVector = impactNormal.scale(-forceMagnitude);
            // apply force at the exact impact point (push asteroid, spin asteroid)
          ...
          drones.splice(i, 1); // delete crashed drone and spawn new one
          this.spawnDrone();
          break;
        }
      }
  ...
  checkEnemyBulletAsteroidCollision(){
    ...
    for(let i = bullets.length - 1; i >= 0; i--){
      ...
      for(const asteroid of asteroids){
        ...
        bullets.splice(i, 1);
        if(this.state.debug.rigidBodiesEnabled && asteroid.applyForceAtPoint){
          const force = bullet.velocity.normalized().scale(1200);
